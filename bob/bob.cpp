...
*/
	...
	save(cipher_matrix,"c2.csv");

	matrix S = getSecretKey(T);
	matrix M2_tmp = S;
	zero(M2_tmp);	
	matrix d(extension,n+extension);
	zero(d);

	matrix S2 = A_on_B(M2_tmp,d);
	matrix T_cloud = load("../cloud/T_alice.csv");
	//cal key switch matrix M2
	s = clock();
	matrix M2 = keySwitchMatrix(transpose(vectorize(S2)),T_cloud);
	t = clock();
	cout<<setprecision(2)<<double(t-s)/CLOCKS_PER_SEC<<endl;
	// save M2
	save(M2,"M2.csv");

	//cal key switch matrix M for Bob's data
	s = clock();
