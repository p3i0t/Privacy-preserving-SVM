...
const matrix keySwitchMatrix(const matrix& S,const matrix& T)
{
	...
	/*
	int t = clock();
	cout<<"getBitmatrix time:"<<double(t-s)/CLOCKS_PER_SEC<<endl;
	...
	cout<<"getBitmatrix time:"<<double(t-s)/CLOCKS_PER_SEC<<endl;
	s = clock();
	matrix up = bitwise_secret_key-(T*A);
	t = clock();
	cout<<"M calcu time:"<<double(t-s)/CLOCKS_PER_SEC<<endl;
	s = clock();
    matrix a = A_on_B(up,A);
	t = clock();
	cout<<"A on B:"<<double(t-s)/CLOCKS_PER_SEC<<endl;
	*/
...
{
	matrix plain = w*x;
	matrix M = keySwitchMatrix(matrix(T.m,'i'),T);
    matrix c = keySwitch(M,plain);
    return c;
}
...
{
    matrix old_key = transpose(vectorize(transpose(S1)*S2));
...
int main(int argc, char * argv[]){
	...
	*/
	...
	matrix* data_a = new matrix[cnt];
	for(int i=0;i<cnt;i++)
	{
		data_a[i] = matrix(n,1);
	}

	matrix* data_b = new matrix[cnt];
	for(int i=0;i<cnt;i++)
	{
		data_b[i] = matrix(n,1);
	}
	...
	*/
    ...
    int t = clock();
    cout<<"Plain Dot Product Calculation time: "<<double(t-s)/CLOCKS_PER_SEC<<endl;	
	*/
	matrix T1(n,extension),T2(n,extension);
	int s = clock();
	// encrypt data from alice
	matrix* cipher_a = new matrix[cnt];
/*	
	matrix plain = w*data_a[0];
	matrix M = keySwitchMatrix(matrix(T1.m,'i'),T1);
     cipher_a[0] = keySwitch(M,plain);
	return 0;
*/
	for(int i=0;i<cnt;i++)
		cipher_a[i] = encrypt(T1,data_a[i],w);
	//encrypt data from bob
	
	matrix* cipher_b = new matrix[cnt];
	for(int i=0;i<cnt;i++)
		cipher_b[i] = encrypt(T2,data_b[i],w);
	/*
	cout<<"c1: "<<endl;
	c1.display();
	*/
    ...
    //matrix H(n,'i');
    s = clock();
	double kernel[cnt][cnt];
	for(int i=0;i<cnt;i++)
		for(int j=i;j<cnt;j++)
	{
			matrix innerProd = innerProduct(M,cipher_a[i],cipher_b[i],w);	
...
/*	
	...
	matrix c1 = encrypt(T,a,w);
	//cout<<"c1"<<endl;
	//c1.display();
	save(c1,"c1.csv");
	matrix S = getSecretKey(T);
	matrix M1_tmp = transpose(S);	
	for(int i=0;i<M1_tmp.n;i++)
		for(int j =0;j<M1_tmp.n;j++)
			M1_tmp.p[i*M1_tmp.n+j] = 0.0;
